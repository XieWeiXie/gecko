...
import (
	"encoding/json"
	"fmt"
	"github.com/xiewei/gogo/v3/types"
	"io/ioutil"
	"net/http"
	"net/url"
	"strconv"
...
func (c *Client) CoinsIDMarketChartRage(id string, vsCurrency string, from string, to string) (*types.CoinsIDMarketChartRange, error) {
	if len(id) == 0 || len(vsCurrency) == 0 || len(from)==0 || len(to)==0  {
		return nil, fmt.Errorf("id, vsCurrency, from and to is required")
	}
	...
	m := types.CoinsIDMarketChartRange{}
	err = json.Unmarshal(resp, &m)
	if err != nil {
		return &m, err
	}
